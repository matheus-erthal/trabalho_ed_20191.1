...
TNoFolha *no_folha(int d, int m, int pont_pai, int pont_prox)
{
	...
	no->clientes = (TPizza **) malloc(sizeof(TPizza *) * 2 * d);
	for (i = 0; i < 2 * d; i++) {
		no->clientes[i] = NULL;
	}
	return no;
...
int cmp_no_folha(int d, TNoFolha *n1, TNoFolha *n2)
{
	...
	for (i = 0; i < n1->m; i++) {
		if (!cmp_pizza(n1->clientes[i], n2->clientes[i])) {
			return 0;
		}
	}
	return 1;
